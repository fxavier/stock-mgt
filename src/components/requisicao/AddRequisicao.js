...
import { ADD_REQUISICAO } from "../../graphql/mutations";
...
const AddRequisicao = () => {
	...
	const [addRequisicao] = useMutation(ADD_REQUISICAO, {
		variables: {
			dataRequisicao,
			provinciaId,
			sectorId,
		...
		update(cache, { data: { addRequisicao } }) {
			const [requisicoes] = cache.readQuery({ query: GET_REQUISICOES });

			cache.writeQuery({
				data: { requisicoes: [...requisicoes, addRequisicao] },
			});
		},
	});
	...
	const handleDataRequisicao = (newDate) => {
		const formattedDate = format(new Date(newDate), "yyyy-MM-dd");
		setDataRequisicao(formattedDate);
	};

	const clearFields = () => {
		setDataRequisicao(format(new Date(new Date()), "yyyy-MM-dd"));
		setInstrumentos([]);
		setProvincias([]);
		setSectores([]);
		setQuantidade(0);
	...
	const onSubmit = (e) => {
		...
		console.log("Data:", dataRequisicao);
